...
class UNetModelSwin(nn.Module):
    ...
    def __init__(self, model_args, **kwargs):
        ...
        channel_mult = model_args.get('channel_mult', (1, 2, 4, 8))
        conv_resample = model_args.get('conv_resample', True)
        dims = model_args.get('dims', 2)
        use_fp16 = model_args.get('use_fp16', False)
        num_heads = model_args.get('num_heads', 1)
        num_head_channels = model_args.get('num_head_channels', -1)
        use_scale_shift_norm = model_args.get('use_scale_shift_norm', False)
        resblock_updown = model_args.get('resblock_updown', False)
        swin_depth = model_args.get('swin_depth', 2)
        swin_embed_dim = model_args.get('swin_embed_dim', 96)
        window_size = model_args.get('window_size', 8)
        mlp_ratio = model_args.get('mlp_ratio', 2.0)
        patch_norm = model_args.get('patch_norm', False)
        cond_lq = model_args.get('cond_lq', True)
        cond_mask = model_args.get('cond_mask', False)
        lq_size = model_args.get('lq_size', 256)
